...
const CreateQuoteModal: React.FC<{
  ...
  React.useEffect(() => {
    ...
    if (editItem) {
      ...
      reset({
        quoteNumber: editItem.quote_number ?? editItem.invoice_number ?? '',
        date: datePart,
        ...
        articles: editItem.articles.map(a => ({
          ...
          tva: parseFloat(String(a.tva_percentage)),
            ...
            <View style={styles.formCard}>
              ...
              <View style={styles.fieldBlock}>
                <Text style={styles.fieldLabel}>
                  {t('label_quote_number')} <Text style={styles.required}>*</Text>
                </Text>
                <Controller
                  control={control}
                  name="quoteNumber"
                  render={({ field: { value } }) => (
                    <TextInput
                      style={[styles.fieldInput, errors.quoteNumber && styles.fieldInputError, { color: '#6B7280', backgroundColor: '#F3F4F6' }]}
                      value={value}
                      editable={false}
                      placeholder={t('placeholder_quote_number')}
                      placeholderTextColor="#9CA3AF"
                    />
                  )}
                />
                {errors.quoteNumber && <Text style={styles.fieldError}>{errors.quoteNumber.message}</Text>}
              </View>

              {/* Date */}
              <View style={styles.fieldBlock}>
                <Text style={styles.fieldLabel}>
                  {t('label_date')} <Text style={styles.required}>*</Text>
                </Text>
                <TouchableOpacity
              ...
              <View style={styles.fieldBlock}>
                <Text style={styles.fieldLabel}>
                  {t('label_due_date')} <Text style={styles.required}>*</Text>
                </Text>
                <TouchableOpacity
                ...
                {errors.validUntil && <Text style={styles.fieldError}>{errors.validUntil.message}</Text>}
              </View>

              {/* Client */}
              <View style={styles.fieldBlock}>
                <Text style={styles.fieldLabel}>
                  {t('label_client')} <Text style={styles.required}>*</Text>
                </Text>
                <TouchableOpacity
                ...
                {errors.clientId && <Text style={styles.fieldError}>{errors.clientId.message}</Text>}
              </View>

              {/* Payment Method */}
              <View style={styles.fieldBlock}>
                <Text style={styles.fieldLabel}>
                  {t('label_payment_method')} <Text style={styles.required}>*</Text>
                </Text>
                <TouchableOpacity
              ...
              <View style={styles.fieldBlock}>
                <Text style={styles.fieldLabel}>
                  {t('label_status')} <Text style={styles.required}>*</Text>
                </Text>
                <TouchableOpacity
                ...
                >
                  <Text style={watchedStatus ? styles.pickerValueText : styles.pickerPlaceholderText}>
                    {watchedStatus ? resolveStatus(watchedStatus, i18n.language) : t('placeholder_status')}
                  </Text>
                  <ChevronDown size={18} color="#1E5BAC" />
                </TouchableOpacity>
                {errors.status && <Text style={styles.fieldError}>{errors.status.message}</Text>}
              </View>
                  ...
                  <View key={i} style={styles.articleRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.articleMeta}>
                        {a.quantity} x {(a.unitPriceHT ?? 0).toLocaleString('fr-FR')} MAD HT
                      </Text>
                    </View>
                  ...
                  {clientSearchLoading ? (
                    ...
                    (clientSearchResults ?? localClients).map(c => (
                      <TouchableOpacity
                        ...
                        onPress={() => { setValue('clientId', c.id, { shouldValidate: true }); closeClientPicker(); }}
                      >
                        <Text style={[styles.pickerOptionText, watchedClientId === c.id && styles.pickerOptionSelected]}>
                          {c.name}
                        </Text>
              ...
              <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: 320 }} showsVerticalScrollIndicator={false}>
                {PAYMENT_METHODS.map(pm => (
                  <TouchableOpacity
                    key={pm.key}
                    style={styles.pickerOption}
                    onPress={() => { setValue('accountId', pm.key, { shouldValidate: true }); setShowAccountPicker(false); }}
                  >
            ...
            <View style={styles.pickerSheet}>
              ...
              {STATUT_OPTIONS.map(s => (
                <TouchableOpacity
                  key={s.key}
                  style={styles.pickerOption}
                  onPress={() => { setValue('status', s.key, { shouldValidate: true }); setShowStatusPicker(false); }}
                >
